...
const CONFIG = {
    ...
    ignoreDirectories: ['node_modules/**', 'dist/**', 'build/**', 'coverage/**', '.git/**'],
    // Bekende development dependencies die vaak ongebruikt lijken
    knownDevDeps: new Set([
        'jest', 'eslint', 'nodemon', 'typescript', '@types/*', 'prettier',
        'supertest', 'mocha', 'chai', 'nyc', 'ts-node', 'webpack', 'babel-*'
    ]),
...
function extractImports(content) {
    ...
    patterns.forEach(({ regex, isDynamic }) => {
        ...
        while ((match = regex.exec(content)) !== null) {
            ...
            if (!packageName.startsWith('.') && !CONFIG.internalModules.has(packageName)) {
                if (isDynamic) {
    ...
    return { imports: Array.from(imports), dynamicImports: Array.from(dynamicImports) };
}
...
async function checkDependencies(projectRoot, packageJsonPath) {
    ...
    if (securityReport.vulnerabilities) {
        ...
        for (const [severity, vulns] of Object.entries(securityReport.vulnerabilities)) {
    ...
    const missing = Array.from(foundImports).filter(imp => !declaredDeps.has(imp));
    if (missing.length === 0) {
        ...
        missing.forEach(dep => {
            ...
            console.log(`- ${dep} (gebruikt in: ${Array.from(files || []).join(', ')})`);
        });
    ...
    const unused = Array.from(declaredDeps).filter(dep => !foundImports.has(dep));
    const unusedDev = unused.filter(dep => 
        CONFIG.knownDevDeps.has(dep) || 
        Array.from(CONFIG.knownDevDeps).some(pattern => 
            pattern.endsWith('*') && dep.startsWith(pattern.slice(0, -1))
        )
    );
    const unusedPeer = unused.filter(dep => CONFIG.knownPeerDeps.has(dep));
    const reallyUnused = unused.filter(dep => 
        !unusedDev.includes(dep) && !unusedPeer.includes(dep)
    );
