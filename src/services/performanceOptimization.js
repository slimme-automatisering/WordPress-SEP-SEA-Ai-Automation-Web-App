...
export class PerformanceOptimizationService {
  async analyzePerformance(url) {
    try {
      ...
      const results = {
        ...
        firstContentfulPaint: this.findMetric(
          performance.metrics,
          "FirstContentfulPaint",
        ),
        largestContentfulPaint: this.findMetric(
          performance.metrics,
          "LargestContentfulPaint",
        ),
        totalBlockingTime: this.findMetric(
          performance.metrics,
          "TotalBlockingTime",
        ),
        cumulativeLayoutShift: this.findMetric(
          performance.metrics,
  ...
  generateRecommendations(metrics) {
    ...
    if (metrics.cumulativeLayoutShift > 0.1) {
      recommendations.push(
        "Improve layout stability by specifying image dimensions",
      );
    }
