...
export class WooCommerceService extends BaseService {
  ...
  async getCustomers(options) {
    ...
    return this.getCached(cacheKey, async () => {
      ...
      const customers = this.validateApiResponse(response.data, 'WooCommerce');
      ...
      const filteredCustomers = customers.filter(customer => {
        ...
        const meetsOrderCount = !options.orderCount || (
          orderCount >= options.orderCount.min &&
          orderCount <= options.orderCount.max
        );

        const meetsTotalSpent = !options.totalSpent || (
      ...
      try {
        switch (entity) {
          ...
          case 'orders':
            ...
            break;
          case 'customers':
            results.details.customers = await this.syncCustomers(options.options);
            break;
          case 'categories':
            results.details.categories = await this.syncCategories(options.options);
            break;
          case 'coupons':
  ...
  async syncCategories(options) {
    const response = await this.api.get('products/categories', { per_page: 100 });
