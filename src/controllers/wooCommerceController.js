...
const schemas = {
  products: Joi.object({
    status: Joi.string().valid('publish', 'draft', 'private').default('publish'),
    category: Joi.string(),
    tag: Joi.string(),
    search: Joi.string(),
    orderBy: Joi.string().valid('date', 'price', 'popularity', 'rating').default('date'),
  ...
  customers: Joi.object({
    role: Joi.string().valid('all', 'administrator', 'customer').default('customer'),
  ...
  sync: Joi.object({
    entities: Joi.array().items(
      Joi.string().valid('products', 'orders', 'customers', 'categories', 'coupons')
    ).min(1).required(),
    options: Joi.object({
...
class WooCommerceController extends BaseController {
  ...
  getProducts = BaseController.asyncHandler(async (req, res) => {
    ...
    return this.sendResponse(res, 200, 'Producten succesvol opgehaald', results.products, {
      total: results.total,
      page: data.page,
  ...
  getOrders = BaseController.asyncHandler(async (req, res) => {
    ...
    return this.sendResponse(res, 200, 'Bestellingen succesvol opgehaald', results.orders, {
      total: results.total,
      page: data.page,
      totalPages: Math.ceil(results.total / data.perPage),
  ...
  getCustomers = BaseController.asyncHandler(async (req, res) => {
    ...
    return this.sendResponse(res, 200, 'Klanten succesvol opgehaald', results.customers, {
      total: results.total,
      page: data.page,
      totalPages: Math.ceil(results.total / data.perPage),
      filters: {
  ...
  syncData = BaseController.asyncHandler(async (req, res) => {
    ...
    return this.sendResponse(res, 200, 'Data synchronisatie succesvol', results, {
