...
const schemas = {
  ...
  realtime: Joi.object({
    metrics: Joi.array()
      ...
      .min(1)
      .default(["activeUsers", "screenPageViews"]),
...
class AnalyticsController extends BaseController {
  ...
  initializeAnalytics = BaseController.asyncHandler(async (req, res) => {
    try {
      ...
      return this.sendResponse(
        res,
        200,
        "Analytics succesvol geïnitialiseerd",
        result,
        {
  ...
  getRealtimeData = BaseController.asyncHandler(async (req, res) => {
    try {
      ...
      return this.sendResponse(
        res,
        200,
        "Realtime data succesvol opgehaald",
        results,
        {
  ...
  getAnalyticsData = BaseController.asyncHandler(async (req, res) => {
    try {
      ...
      return this.sendResponse(
        res,
        200,
        "Analytics data succesvol opgehaald",
        results,
        {
  ...
  exportAnalyticsData = BaseController.asyncHandler(async (req, res) => {
    try {
      ...
      res.setHeader(
        "Content-Disposition",
        `attachment; filename="analytics-export-${Date.now()}.${data.format}"`,
      );
