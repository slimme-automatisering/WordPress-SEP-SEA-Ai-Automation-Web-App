...
async function activateLicense(req, res) {
  ...
  if (!key || !companyName) {
    return res
      .status(400)
      .json({ error: "Licentiesleutel en bedrijfsnaam zijn verplicht" });
  }
...
async function deactivateLicense(req, res) {
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res
        .status(404)
        .json({ error: "Licentie niet gevonden of al gedeactiveerd" });
    }
...
async function upgradeLicense(req, res) {
  ...
  if (!key || !newType) {
    return res
      .status(400)
      .json({ error: "Licentiesleutel en nieuw type zijn verplicht" });
  }
  ...
  try {
    ...
    if (currentLicense.rows.length === 0) {
      ...
      return res
        .status(404)
        .json({ error: "Licentie niet gevonden of niet actief" });
    }
