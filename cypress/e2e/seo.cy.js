  ...
  describe('Keyword Analyse', () => {
    it('moet keywords kunnen analyseren', () => {
      ...
      cy.request({
        ...
        expect(response.body.data).to.have.property('keywords').that.deep.equals(keywords);
  ...
  describe('Keyword Suggesties', () => {
    it('moet suggesties kunnen ophalen', () => {
      cy.request({
        ...
        expect(response.body.data).to.have.property('suggestions').that.is.an('array');
      });
    });
    ...
    it('moet rate limiting toepassen', () => {
      // Maak meer requests dan toegestaan
      const requests = Array(201).fill().map(() => {
        return cy.request({
          method: 'GET',
          url: '/api/v1/seo/suggestions',
          qs: { keyword: 'test' },
